    ...
    <div class="panel current-session-panel">
      ...
      <div v-if="currentConnection" class="connection-body">
        ...
        <div class="session-row">
          <span>浏览器标识</span
          ><code>{{ currentConnection.browserId ?? "由平台状态管理" }}</code>
        </div>
        <div class="session-row">
        ...
        <div class="session-row">
          <span>连接开始</span
          ><strong>{{ formatDate(currentConnection.startedAt) }}</strong>
        </div>
        <div class="session-actions">
          <el-button
            type="primary"
            :icon="RefreshRight"
            @click="runAction('start', [], 'Codex Dream Skin 已启动或重启。')"
            >重启并应用</el-button
          >
        </div>
      </div>
    ...
    <div class="panel history-panel">
      <div class="panel-head">
        ...
        <div class="session-list-actions">
          ...
          ><el-tooltip content="刷新 Codex 会话" placement="top"
            ><el-button
              class="icon-button"
              :icon="Refresh"
              circle
      ...
      <div v-if="codexSessions.length" class="codex-session-list">
        <section
          v-for="group in codexSessionGroups"
          :key="group.key"
          class="session-group"
        >
          ...
          >
            ...
            <div class="history-copy">
              ...
              ><span
                >{{ formatDate(session.updatedAt) }} · {{ session.id }}</span
              >
            </div>
