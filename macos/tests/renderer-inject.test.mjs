...
assert.match(css, /\[data-app-shell-page-header\]\s+\[data-app-shell-header-toolbar\]\s*>\s*:first-child\s*\{[\s\S]{0,320}background:\s*transparent !important;[\s\S]{0,320}transition:\s*none !important;/,
  "The macOS conversation title surface must not restore a solid background while a new thread mounts.");
